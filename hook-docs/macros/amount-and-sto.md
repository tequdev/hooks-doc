...
#define AMOUNT_TO_DROPS(amount_buffer)\
     (((amount_buffer)[0] >> 7) ? -2 : ( /* decode 8-byte big-endian XAH drops */ ))
```

Decode the 8-byte serialized **native XAH** Amount payload into an `int64_t` drop
count. **Expression.** Returns the drop value on success, or `-2` if the high bit of
byte 0 is set — which in Xahau Amount encoding means the value is *not* native XAH (it is
...
int64_t drops = AMOUNT_TO_DROPS(amt);  // >=0 for XAH, -2 if the amount is an IOU
if (drops < 0)
    NOPE("expected native XAH");
```

Caveats:
- Pass the **8-byte Amount payload** (for XAH the serialized amount is 8 bytes; an IOU
...
- The mask constant in the definition is written `0xb00111111`; it reads like an
  ...
  For all valid XAH drop amounts the two encodings produce the same result, but if you
