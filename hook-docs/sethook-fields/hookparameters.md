...
`soeOPTIONAL` on `sfHook` (`src/libxrpl/protocol/InnerObjectFormats.cpp:96`);
`soeREQUIRED` on `ltHOOK_DEFINITION`
(`include/xrpl/protocol/detail/ledger_entries.macro:101` — a definition
always carries its (possibly empty) default array). Each element is an
`sfHookParameter` object, `OBJECT` code 23 (`sfields.macro:374`), with this
template (`InnerObjectFormats.cpp:111-114`):
...
respectively (`sfields.macro:287-288`).

**Validation — `validateHookParams` (`src/xrpld/app/tx/detail/SetHook.cpp:115-205`),
run for `hsoINSTALL`, `hsoUPDATE`, and `hsoCREATE`** whenever the array is
present (`SetHook.cpp:341-344,385-388,414-418`); forbidden entirely on
`hsoDELETE`/`hsoNSDELETE` (`SetHook.cpp:265,298`):

1. **Max 16 entries** (`HOOK_PARAMS_COUNT`, `SetHook.cpp:121-129`), verified
   `SetHook_test.cpp:485-508` (17 params → `temMALFORMED`).
2. **Every element must be an `sfHookParameter` object**
   (`PARAMETERS_ILLEGAL`, `SetHook.cpp:135-142`), verified
   `SetHook_test.cpp:571-588`.
3. **Every element's sub-fields must only be `sfHookParameterName`/`Value`**
   (`PARAMETERS_FIELD`, `SetHook.cpp:148-160`), and `sfHookParameterName`
   must actually be present (`PARAMETERS_NAME`, `SetHook.cpp:166-174`).
4. **Size limits** — name ≤ `hook::maxHookParameterKeySize()` = 32 bytes,
   ...
   `HOOK_PARAM_SIZE`), verified `SetHook_test.cpp:531-568` (33-byte name,
   257-byte value both rejected).
5. **No duplicate names within the submitted array**
   (`PARAMETERS_NAME_REPEATED`, `SetHook.cpp:191-201`), verified
   `SetHook_test.cpp:510-529`.
...
**The three-value semantics of a submitted parameter — `updateHookParameters`
(`SetHook.cpp:1105-1256`), run at apply time for `hsoUPDATE` and
`hsoINSTALL`.** A parameter in a `SetHook`'s `sfHookParameters` array can
take one of three shapes, and each means something different
(`SetHook.cpp:1117-1136`, comment preserved because the distinction is easy
to lose):
...
The algorithm (`SetHook.cpp:1138-1213`): start from the entry's existing
stored parameters (`oldParameters` — empty for `hsoINSTALL`, since there is
no prior entry), overlay every name from the submitted array (AV becomes an
unpopulated `std::optional`, EV becomes a populated-but-empty one), then for
each name that also has a definition default: drop it from the result if
the resolved value is absent-or-equal-to-default, or keep it as an explicit
blank if it's an EV that differs from a default that does exist
(`explicitBlanks`, `SetHook.cpp:1169-1194`). Finally, anything still
AV-or-EV-without-a-default is dropped entirely (`SetHook.cpp:1201-1213`).
The 16-entry cap and the per-entry size limits are re-checked on the
*merged* result too (`SetHook.cpp:1215-1244`, `tecINTERNAL` on failure —
this is a defense-in-depth re-check, not expected to trigger given
preflight validation already bounded the input).

**`hsoCREATE` just stores the submitted array as the definition's default,
unmerged** (`SetHook.cpp:1861-1865`):
...
There is no three-way merge on create — every name/value pair supplied
becomes the baseline every later install/update is diffed against.

**Owner reserve vs. transaction fee — two different charges.**
`SetHook::computeHookReserve` (`SetHook.cpp:1264-1279`) adds 1 owner-reserve
unit per parameter that ends up stored on the account's `sfHook` entry
(same as grants — see [HookGrants](hookgrants.md)). Separately,
`SetHook::calculateBaseFee` (`SetHook.cpp:657-683`) adds a **flat 1 drop per
byte** of name+value in the transaction's *submitted* `sfHookParameters`
array to the transaction fee — charged once at submission regardless of
whether the merge above later discards some entries as matching defaults.
The code comment above this block claims parameters are "billed at the
same rate as code bytes" (`SetHook.cpp:658`), but `sfCreateCode` is billed
500 drops/byte via `hook::computeCreationFee` (see
[CreateCode](createcode.md)) — the comment does not match the 1-drop/byte
arithmetic actually implemented; treat the code as authoritative.

**Verified examples (`SetHook_test.cpp:2597-2796`):**
