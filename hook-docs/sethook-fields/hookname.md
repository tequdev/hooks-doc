...
**Storage — `ltHOOK` only, never `ltHOOK_DEFINITION`.** Confirmed directly by
`SetHook_test.cpp:1760-1766`:
...
while the per-account `ltHOOK` entry does carry it
(`SetHook_test.cpp:1767-1775`). This makes sense: the same WASM code (one
...
(`include/xrpl/protocol/detail/features.macro:39`, `Supported::yes,
VoteBehavior::DefaultNo`) gates it in **two different places**, with two
**different** rejection codes:

1. Inside a `SetHook`'s `sfHook` object — `temDISABLED` if present without the
   amendment (`SetHook.cpp:815-817`):
   ```cpp
...
2. As the common transaction-level field (any transaction type) —
   ...
   (`Transactor.cpp:152-160`, in the shared `preflight1`):
   ```cpp
   ...
   ```
   This same function call also enforces the length/UTF-8 rule below for
   *every* transaction carrying `sfHookName`, not only `SetHook`.

**Validation rules — `SetHook::validateHookName`
(`SetHook.cpp:624-639`):**
...
must also be valid UTF-8 (`URIToken::validateUTF8`) — a purely random 4-byte
blob like `DEADBEEF` (hex) fails this even though it passes the length check
(`SetHook_test.cpp:1718-1723`, `"DEADBEEF" // not utf-8`); `"41424344"` (ASCII
`ABCD`) is the value used throughout the passing tests.

**Runtime semantics — targeting, not filtering.** When a hook chain executes
(`Transactor::executeHookChain`, `Transactor.cpp:1357-1369`, and identically in
`calculateHookChainFee`, `Transactor.cpp:286-298`), each `sfHook` entry with a
...
**Verified example (`SetHook_test.cpp:1864-1930`):** account `alice` has two
...
chains, per the same test file's "Test Weak" and "Callback Execution" sections
(`SetHook_test.cpp:1932-2032`). Note the emitted callback transaction itself
does **not** need to carry `sfHookName` to trigger `cbak` on the named hook
(`SetHook_test.cpp:2029-2031`, "Callback transaction doesn't need to have hook
name").
...
- Passing a name that fails UTF-8 validation despite being 4–16 bytes long
  (e.g. arbitrary binary/hex blobs) — `validateHookName` checks both
