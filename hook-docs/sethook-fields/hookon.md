...
create — omitting it is rejected. From `SetHook.cpp:458-469`:
...
value is not read from a single field — `hook::getHookOn` (`applyHook.cpp:850-865`)
...
before an implicit zero. This lets `SetHook.cpp` avoid storing a value on
...
This is exactly the value used (as `HookOnOutgoing`) in
`src/test/app/SetHook_test.cpp:1564-1566`, commented `// Payment high`, and its
zero-bit positions verify to `{0, 22}` — bit 0 clear (fires on `ttPAYMENT`), bit
22 clear (does not fire on `ttHOOK_SET`). The paired `HookOnIncoming` value in
the same test (`SetHook_test.cpp:1561-1563`, commented `// Invoke high`) clears
bits `{22, 99}` — `ttINVOKE = 99` (`transactions.macro:557`) — for the same
reasoning applied to `ttINVOKE` instead of `ttPAYMENT`.
...
- Forgetting that a generic `HookOn` on the `sfHook` object silently overrides
  *both* directions even if the definition has distinct
  `HookOnIncoming`/`HookOnOutgoing` values — see the "Execution" test at
  `SetHook_test.cpp:1600-1656`, where installing a hook with `HookOn = 0` at the
  `ltHOOK` level makes it fire on both incoming and outgoing traffic regardless
  of the definition's split values.
