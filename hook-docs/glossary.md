...
| **TSH (Transactional Stake Holder)** | An account with a stake in a transaction (beyond the sending account) that can therefore have its hooks fire for that transaction. TSHs may be added as **weak** (observe only) or strong participants. Governed by `TSHFlags` (`tshROLLBACK`, `tshCOLLECT`, `tshMIXED`). **Unverified — needs confirmation:** the precise strong-vs-weak TSH firing rules per transaction type are not fully restated here.<!-- in Enum.h; the SetHookTSH_test.cpp suite exercises weak TSHs (addWeakTSH); see SetHookTSH_test.cpp --> |
