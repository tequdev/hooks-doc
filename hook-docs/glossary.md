...
| **Burden** | A measure of how much emitted-transaction "work" descends from a transaction. For the originating transaction it is 1 unless that transaction was itself emitted by a previous hook. Read with `otxn_burden` / `etxn_burden`. See [api-reference/transaction.md](api-reference/transaction/README.md).<!-- comment on otxn_burden in applyHook.cpp --> |
...
| **Callback** | The execution mode in which `cbak` runs to deliver the outcome of an emitted transaction (emission flag `hefCALLBACK = 0x2`). See [overview.md](overview.md).<!-- isCallback/emitFailure in applyHook.h --> |
| **Collect call** | A weak-TSH hook execution: it runs after the transaction has applied and cannot change the outcome. Requires both the TSH account's `asfTshCollect`/`lsfTshCollect` flag and the individual hook's `hsfCOLLECT` flag. See [tsh.md](tsh.md).<!-- src/xrpld/app/tx/detail/Transactor.cpp:1751-1763, 1380-1390 --> |
| **drops** | The smallest unit of XAH/native balance (1 XAH = 1,000,000 drops). Base fee is expressed in drops (`fee_base`). See [api-reference/ledger-and-slot.md](api-reference/ledger/README.md). |
...
| **Execution metadata** | The record of a hook's execution written into transaction metadata: an `sfHookExecutions` array of `sfHookExecution` objects carrying `sfHookResult`, `sfHookReturnCode`, `sfHookReturnString`, `sfHookInstructionCount`, `sfHookEmitCount`, and `sfHookExecutionIndex`. See [overview.md](overview.md).<!-- built in applyHook.cpp --> |
| **Generation** | A counter of how many emit "generations" deep a transaction is; 1 unless the originating transaction was itself emitted. Read with `otxn_generation` / `etxn_generation`. See [api-reference/transaction.md](api-reference/transaction/README.md).<!-- comment on otxn_generation in applyHook.cpp --> |
...
| **Guard** | The `_g(guard_id, maxiter)` call required at the top of every loop, bounding its iterations. Missing/invalid guards are rejected at HookSet time (log codes `GUARD_IMPORT`/`GUARD_MISSING`/`GUARD_PARAMETERS`); exceeding a guard at runtime returns `GUARD_VIOLATION` (`-16`). See [api-reference/control.md](api-reference/control/README.md) and [macros.md](macros/guards.md).<!-- Guard.h --> |
...
| **Hook Parameter** | A key/value pair configured on a hook at install time and read at runtime with `hook_param`. Keys are up to 32 bytes, values up to 256 bytes, max 16 per hook. Distinct from parameters carried on the originating transaction (`otxn_param`). See [api-reference/control.md](api-reference/control/README.md).<!-- Enum.h --> |
...
| **`hsfOVERRIDE`** | A `SetHook` flag (`0x01`) required to override or delete an existing hook. Without it, create/delete operations are rejected (log codes `CREATE_FLAG`, `DELETE_FLAG`, `OVERRIDE_MISSING`). See [overview.md](overview.md).<!-- Enum.h --> |
...
| **Strong execution** | The pre-apply execution mode: the hook runs before the transaction is applied and may `rollback` to reject it (emission flag `hefSTRONG = 0x1`). See [overview.md](overview.md).<!-- isStrong in applyHook.h --> |
| **Strong TSH** | A TSH nominated with `tshSTRONG`/`tshROLLBACK`: its hook chain runs before the transaction applies, alongside the originating account's, and can `rollback` the transaction. See [tsh.md](tsh.md).<!-- src/xrpld/app/hook/detail/applyHook.cpp:59; src/xrpld/app/tx/detail/Transactor.cpp:1975-2013 --> |
| **TSH (Transactional Stake Holder)** | An account, other than the originating account, that a transaction affects closely enough that its own installed hooks are also given a chance to run for that transaction. Computed per-transaction by `hook::getTransactionalStakeHolders`, and added as either a **[Strong TSH](#strong-tsh)** (can rollback) or a **[Weak TSH](#weak-tsh)** (collect call only). See [tsh.md](tsh.md) for the full mechanism and a per-transaction-type table.<!-- src/xrpld/app/hook/detail/applyHook.cpp:27-595 --> |
| **Weak execution** | A post-apply re-execution requested by a strong hook via `hook_again()`. It observes the applied result and cannot roll it back. See [overview.md](overview.md).<!-- executeAgainAsWeak comment in applyHook.h --> |
| **Weak TSH** | A TSH nominated with `tshWEAK`/`tshCOLLECT`: its hook chain runs only as a [Collect call](#collect-call) after the transaction has applied, gated on the account's `asfTshCollect` flag and the hook's `hsfCOLLECT` flag. See [tsh.md](tsh.md).<!-- src/xrpld/app/hook/detail/applyHook.cpp:60; src/xrpld/app/tx/detail/Transactor.cpp:1751-1763, 1380-1390 --> |
