...
int64_t hook(uint32_t r)
{
    ...
    // 1. set the transaction-specific fields
    SET_NATIVE_AMOUNT(AMOUNT_OUT, 1000);   // 1000 drops
    SET_ACCOUNT(DEST_OUT, bob);

    // 2. PREPARE_TXN() reserves emission capacity, inserts sfEmitDetails via
    //    etxn_details, and prices the transaction via etxn_fee_base.
    PREPARE_TXN();

    // 3. emit and capture the resulting transaction hash
    uint8_t hash[32];
    if (emit((uint32_t)hash, 32, (uint32_t)txn, sizeof(txn)) != 32)
