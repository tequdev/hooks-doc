...
- **`flags == 1`** — the slot must hold an `STI_AMOUNT`; returns `1` if the amount is native
  (XRP) and `0` if it is an IOU. If the slot is not an amount, returns `NOT_AN_AMOUNT` (-32).
...
- Use `flags == 1` to branch between XRP and IOU handling before calling
  [`slot_float`](slot_float.md).

**Minimal example.**

```c
int64_t is_xrp = slot_type(amt_slot, 1);   // 1 = XRP, 0 = IOU
```
