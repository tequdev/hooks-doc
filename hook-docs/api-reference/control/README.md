...
| [`hook_param_set`](hook_param_set.md) | Override a parameter by hook hash; the hash is stored verbatim (no chain-order lookup). <!-- evidence: `HookAPI::hook_param_set` stores overrides under the target hash without a chain-order check (`src/xrpld/app/hook/detail/HookAPI.cpp:1712-1741`). --> |
